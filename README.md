...
```bash
kubebuilder init --domain arthurvardevanyan.com --repo github.com/ArthurVardevanyan/k8s-federated-credential-operator
kubebuilder create api --group federatedcredential --version v1beta1 --kind Token --namespaced=true
```
...
```sh
make docker-build docker-push IMG=<some-registry>/k8s-federated-credential-operator:tag
```

3. Deploy the controller to the cluster with the image specified by `IMG`:

```sh
make deploy IMG=<some-registry>/k8s-federated-credential-operator:tag
```
